...
$(document).ready(function() {
	$( "#Ingreso_orden_imprimir" ).click(function() {
		...
		$.ajax({
			type: "POST",
			url: "select/trae_orden_modificar.php",
			data:stream,
			success: function(data)	{	
				if (data.indexOf("Error")==-1)
				{
					var numero_orden = data;
					location.href = "orden_compra_modificar.php?numero_orden="+numero_orden;
				}
				else
				{
					alert ("Orden No Se Puede Modificar");
				}
			}			
		}); 
	});
