...
<head>
	...
	<title>Orden de Despacho Nacional</title>
...
$(document).ready(function() {	
	$('#num_nv_despacho').keypress(function (e) {
		if(e.which ==13)
		{
			if($.trim($("#num_nv_despacho").val())==="0") 
			{
				$("#num_nv_despacho").focus();
				$('#valida-proforma_0').fadeIn('slow'); 
				setTimeout(function(){$('#valida-factura_0').fadeOut('slow');},1000); 
				$("#num_proforma_despacho").val('');
				return false;
			}
			if($.trim($("#num_nv_despacho").val())==="") 
			{
				$("#num_nv_despacho").focus();
			...
			var numero_nota_venta=$("#num_nv_despacho").val();
					...
					<div class="content">  
						...
						<div>  
					 	...
					 	<table class="tableform">
							...
							<tr>
								<td>
									<label>Numero Nota de Venta</label>
								</td>
							</tr>
							<tr>
								<td>
									<input type="text" id="num_nv_despacho" placeholder="Numero de Proforma"/>	
