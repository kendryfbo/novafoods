<?php	 
	...
	else if ($funcion==2)
	{
		...
		$id_cliente_nacional=trim($_POST["id_cliente_nacional"]);
                $id_suc_cli=trim($_POST["id_suc_cli"]);
                $id_condicion_venta=trim($_POST["id_pago_cli"]);
		...
		$fecha_despacho_nota_venta_2=date("Y-m-d",strtotime($fecha_despacho_nota_venta));
                ...
                $id_Usuario=trim($_POST["id_Usuario"]);	
                $numero_nota_venta=1;
                $sql5="SELECT max(numero_nota_venta) FROM nota_venta where id_centro_venta=".$id_centro_venta;
			$resultado5=mysql_query($sql5,$conexion->link);
			while ($fila5 = mysql_fetch_array($resultado5))
			{
					$numero_nota_venta=$fila5[0]+1;
		...
		try{
                        $sql3="INSERT INTO nota_venta (numero_nota_venta,id_cliente,fecha_emision,id_centro_venta,sub_total,iva,total,total_ila,id_vendedor,version,orden_compra_externa,ingresada,fecha_despacho,id_condicion,observacion_despacho,id_usuario,estado,suc_cliente)
				VALUES ('$numero_nota_venta','$id_cliente_nacional','$fecha_nota_venta_2','$id_centro_venta','$subtotal','$iva','$total_nota_venta','$ila','$id_vendedor','$version','$orden_compra','Si','$fecha_despacho_nota_venta_2','$id_condicion_venta','$observacion_despacho','$id_Usuario','0','$id_suc_cli')";
			...
			/*if ($sql3) {
                        ...
                        $numero_crl=1;
                        $sql2="SELECT max(numero) FROM nota_venta";
			...
			while ($fila2 = mysql_fetch_array($resultado2))
			{
					//$numero_crl=$fila2[0]+1;
                        ...
                        //echo "numero".$numero_crl;
                        
			$sql1="SELECT numero_nota_venta,Cantidad,id_producto,Precio,total,ingresado_factura,descuento_procentaje,descuento,ila
			FROM temporal_detalle_nota_venta
			where id_usuario=".$id_Usuario;
			$resultado1=mysql_query($sql1,$conexion->link);
			while ($fila1 = mysql_fetch_array($resultado1))
			{
				$sql4="INSERT INTO detalle_nota_venta (numero_nota_venta,Cantidad,id_producto,Precio,total,ingresado_factura,descuento_procentaje,descuento,ila,crl_nota_venta)
				VALUES ('$numero_nota_venta','$fila1[1]','$fila1[2]','$fila1[3]','$fila1[4]','$fila1[5]','$fila1[6]','$fila1[7]','$fila1[8]','$numero_crl')";
				...
				$resultado=mysql_query($sql,$conexion->link);
                        ...
                        $sql8="SELECT centro_venta FROM centro_venta where id_centro_venta=".$id_centro_venta;
                        //$sql2="SELECT numero FROM nota_venta where numero_nota_venta=".$numero_nota_venta;
			...
			echo "Nota de Venta de ".$nombre_cv." Numero ".$numero_nota_venta ."Grabada";
		}
			catch (Exception $e)
		{    
			echo $e->getMessage();
	...
	else if ($funcion==3)
	{
		...
		$sql2="select 
				...
				where id_cliente=".$id_cliente;*/
				$ejecuta2=mysql_query($sql2,$conexion->link);
		$numero_filas1 = mysql_num_rows($ejecuta2);
		if ($numero_filas1<>0)
		{
			$fila2=mysql_fetch_array($ejecuta2);
			echo $fila2[0];
	...
	else if ($funcion==4)
	{
		...
		echo "<select id='lista_precio1' class='limpiar'  onClick='$(this).fil_lista_precio();'>";
		echo "<option id=''  selected>Seleccione Lista de Precio</option>";	
		while ($fila1 = mysql_fetch_array($ejecuta2))
		{
			echo "<option id=".$fila1[0]." >".utf8_encode($fila1[1])."</option>";
                        //echo "<option id=".$fila1[0]." >LP ".$fila1[0]." ".$fila1[1]."</option>";
		}
		echo "</select>";
		echo "<div id='valida-precios' style='display:none' class='errores'>Debe Seleccionar una Lista de Precio</div>";
                /*$sql2="select 
				Numero_lista,
				DATE_FORMAT(fecha,'%d/%m/%y') as fecha
				from 
				lista_precio_nacional
				group by Numero_lista";	
		$ejecuta2=mysql_query($sql2,$conexion->link);
		echo "<select id='lista_precio' class='limpiar'>";
		echo "<option id=''  selected>Seleccione Lista de Precio</option>";	
		while ($fila1 = mysql_fetch_array($ejecuta2))
		{
			echo "<option id=".$fila1[0]." >LP ".$fila1[0]." ".$fila1[1]."</option>";
		...
		echo "</select>";
		echo "<div id='valida-precios' style='display:none' class='errores'>Debe Ingresar Lista de Precio</div>";*/
	}
	else if ($funcion==5)
	{
		$numero_nota_venta=trim($_POST["numero_nota_venta"]);
